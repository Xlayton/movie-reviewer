<template>
  <div class="review-data" @mouseover="isHovering = true" @mouseout="isHovering=false">
    <button v-if="isEditable && !enableEdit" :class="{hovered: isHovering}" v-on:click="() => this.enableEdit = true" class="edit-button">Edit Review</button>
    <h1>{{user_id}}</h1>
    ...
    <button v-if="enableEdit" v-on:click="() => {editReview(index); this.enableEdit = false}">Update</button>
  </div>
</template>
