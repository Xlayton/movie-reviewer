...
create table if not exists reviews(id int auto_increment primary key, user_id int not null, movie_id int, review_body varchar(250), movie_rating int unsigned check (movie_rating <11), foreign key (user_id) references users(id) on delete cascade)
